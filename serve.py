...
- 이모티콘(이모지)은 카페 공지에 어울리게 **조금 넉넉히** 써도 됩니다. 핵심 문장·소제목·마무리 인사 등에 골고루 한두 개씩 붙여 분위기를 살리되, **문단마다 줄줄이 붙이거나** 남발·유행어·과한 장식은 피합니다.
  · 다정하게: 문단·항목마다 친근함이 느껴지도록 여러 곳에 소량 배치(예: 💛✨🙏 등, 한 문단에 이모지 여러 개 연달아 나열은 자제).
  · 부드럽게: 부담 없이 은은하게 곳곳에 가볍게(예: 🌿💬😊 등).
  · 정중하게: 격을 해치지 않게 절제하되, 제목·안내 포인트에는 가끔 붙여도 됨(예: 📋✔💡 등).
